...
namespace Orcus.Administration.Extensions
{
    ...
    {
        public static readonly DependencyProperty UpperTextProperty = DependencyProperty.RegisterAttached("UpperText",
            typeof (string), typeof (TextBlockExtensions), new UIPropertyMetadata(string.Empty, OnUpperTextChanged));

        public static readonly DependencyProperty PlaceHolderTextProperty = DependencyProperty.RegisterAttached(
            "PlaceHolderText", typeof (string), typeof (TextBlockExtensions), new PropertyMetadata(default(string)));

        public static readonly DependencyProperty FormattedTextProperty = DependencyProperty.RegisterAttached(
            "FormattedText", typeof (string), typeof (TextBlockExtensions),
            new PropertyMetadata(default(string), PropertyChangedCallback));

        public static string GetUpperText(DependencyObject obj)
        {
            return (string) obj.GetValue(UpperTextProperty);
        }

        public static void SetUpperText(DependencyObject obj, string value)
        {
            obj.SetValue(UpperTextProperty, value);
        }
        ...
        public static void SetPlaceHolderText(DependencyObject element, string value)
        {
            ...
            DependencyPropertyDescriptor.FromProperty(TextBlock.TextProperty, typeof (TextBlock))
                .RemoveValueChanged(txt, TextChangedHandler);
            DependencyPropertyDescriptor.FromProperty(TextBlock.TextProperty, typeof (TextBlock))
                .AddValueChanged(txt, TextChangedHandler);
            ...
            DependencyPropertyChangedEventArgs dependencyPropertyChangedEventArgs)
        {
                        ...
                        .Split(new[] {"\r\n", "\n"}, StringSplitOptions.None))
            {
                var tmpLine = line;
                bool success = false;
                foreach (var collection in from rule in rules
                    let regex = new Regex(rule.RegexPattern)
                    let match = regex.Match(tmpLine)
                    where match.Success
                    select rule.GetRun(match))
                {
