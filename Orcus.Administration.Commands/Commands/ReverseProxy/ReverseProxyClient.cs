...
{
    public class ReverseProxyClient : INotifyPropertyChanged
    {
        ...
        private const int SOCKS5_DEFAULT_PORT = 3218;
        ...
        public ReverseProxyClient(Socket socket)
        {
            Socket = socket;
        ...
        public event EventHandler<ReverseProxyConnect> Connect;
        public event EventHandler Disconnected;
        public event EventHandler<byte[]> SendDataPacket;
        ...
        public void SendToClient(byte[] payload, int index, int length)
        {
            lock (Socket)
            {
                try
                {
                    BytesSent += length;
                    Socket.Send(payload, index, length, SocketFlags.None);
                }
                catch
                {
                    Disconnect();
        ...
        public void HandleCommandResponse(ReverseProxyStatusUpdate response)
        {
